...
async def fast_upload(client, file_location, reply=None, name=None, progress_bar_function = progress_bar_str, progress_args: tuple = tuple()):
    timer = Timer()
    if name == None:
        name = file_location.split("/")[-1]
    async def progress_bar(downloaded_bytes, total_bytes, started_at):
        # if timer.can_send():
       await progress_bar_function(downloaded_bytes, total_bytes, *progress_args, started_at)
